...
func TestInsert(t *testing.T) {
	...
	assert.Equal(b.Nodes(), len(data), "Counting tree nodes, expected %d, not %d", len(data), b.Nodes())
...
func TestDelete(t *testing.T) {
	...
	b.Delete(37)
	assert.Equal(b.Nodes(), 5, "After deleting, tree should have 5 nodes, go %d", b.Nodes())
	...
	assert.False(b.Contains(72), "Delete 37 deos not work.")
}
...
func TestFind(t *testing.T) {
	...
	assert.Nil(b.Find(999), "Find a non-exist element should return nil")
}
...
func TestWalk(t *testing.T) {
	...
	assert.Equal([]int{2, 10, 37, 56, 72, 89}, items, "walk a tree should give the exect element as inserted.")
}