...
// consume reads the msga channel
...
func main() {
	...
	//  get the maximum numbr of messages from flags
	max := flag.Int("n", 5, "defines the number of messages")
	flag.Parse()

	// utilze the max num of cores available
	...
	if *memprofile != "" {
		...
		if err != nil {
			log.Fatal("could net create memory profile: ", err)
		}
		runtime.GC() //get up-to-date statistics
		if err := pprof.WriteHeapProfile(f); err != nil {
			log.Fatal("could net write memory profile: ", err)
		}
		f.Close()
	}
}